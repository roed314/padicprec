...
\begin{itemize}
\item Bostan et al. \cite{Bostan} use Newton sums for polynomials over $\Zp$ to compute composed products for polynomials over $\Fp$;
\item Gaudry et al. \cite{Gaudry} use $p$-adic lifting methods to generate genus 2 CM hyperelliptic curves;
\item Kedlaya \cite{Kedlaya} uses $p$-adic cohomology to count points on hyperelliptic curves over finite fields;
\item Lercier and Survent \cite{Lercier} compute isogenies between elliptic curves using $p$-adic differential equations.
\end{itemize}
...
Even a long sequence of operations may not sacrifice any precision.  Intervals
...
in implementations of $p$-adic numbers.  % Add citations to Sage, Magma and PARI.
...
\todofor{Xavier}{Extend Lemma \ref{lem:main} to manifolds.}
