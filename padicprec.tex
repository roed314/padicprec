...
differentiable manifolds locally modeled on (eventually infinite 
dimensional) ultrametric Banach spaces. This covers all the 
...
\For {$i$ from $1$ to $n-3$}{
  ...
  {\bf lift} (arbitrary) 
    $b$, $c$ and $d$ to precision $O(p^{\prec})$\;
...
= f \circ \cdots \circ f$ ($i$ times). Apparently $f$ is strictly 
differentiable on $\Q_p^\star \times \Q_p^3$ and its differential in the 
...
at $(u_0, u_1, u_2, u_3)$ and the matrix its differential at this point 
is:
$$D_i = D(u_{i-1}, u_i, u_{i+1}, u_{i+2}) \cdots
D(u_1, u_2, u_3, u_4) \cdot D(u_0, u_1, u_2, u_3).$$
%Its determinant is:
%$$\det D_i = \frac{u_{i+3}}{u_{i-1}} \cdots 
%\frac{u_5}{u_1} \cdot \frac{u_4}{u_0} =
%\frac{u_i \cdot u_{i+1} \cdot u_{i+2} \cdot u_{i+3}}
%{u_0 \cdot u_1 \cdot u_2 \cdot u_3}.$$
Moreover thanks to the Laurent phenomenon, we know that $D_i$ has
integral coefficients. Hence, letting $\varphi_i = f'_i(u_0, u_1, u_2, 
u_3)$, we get 
$$(\det D_i) \cdot p^N \Z_p^4 \subset \varphi_i(p^N \Z_p^4) 
\subset p^N \Z_p^4$$
for all integer $N$.
