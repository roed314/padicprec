...
exhibits the \emph{Laurent phenomenon} \cite{fomin-zelevinsky:02a}. This means that, for all integer 
...
\begin{enumerate}
\item compute the Laurent polynomials $P_n$ using the induction formula (and working in 
...
Let $r \in (0, \delta)$. Let $B^-_E(\rho r) \subset H \subset B_E(r).$ Let us 
...
We refer to Schneider \cite{schneider:11a}, \S 6, for a definition of a locally analytic function, and we extend it to any complete $K$ vector space :

\begin{deftn} \label{deftn:analyticNP}
Let $E$ and $F$ be two complete ultrametric $K$-Banach spaces. Let $U \subset E$ be an open set.
A function $f \: : \: U \rightarrow F$ is said to be locally analytic if for any $x \in U,$ there exists some open subset $U_x$ of $E$ and some $n$-linear mappings $L_n \: : \: E^n \rightarrow F$ for any $n \geq 1,$ such that : for any $y \in U_x,$ the sum $\sum_{n \geq 1} L_n(y,\dots,y)$ converges in F, and $f(x+y)=f(x)+\sum_{n \geq 1} L_n(y,\dots,y).$
\end{deftn}

We then mostly study such a serie defined at $x$.

\begin{deftn} \label{deftn :  serie and norm}
Let $f(x)= \sum_{n \geq 0} f_n (x)$ be a serie over mappings $E \rightarrow F,$ where $E$ and $F$ are complete normed $\overline{K}$-vector space. We assume that for any $\lambda  \in \overline{K},$ $x \in E,$ $n \in \mathbb{N},$ $\vert f_n(\lambda x) \vert = \vert \lambda \vert \vert f_n (x) \vert.$ 
We define $\vert f_n \vert = \sup_{x \in B_E(0,1)} \vert f_n(x) \vert,$ and we assume that all the $\vert f_n \vert $ are finite. 
We also define, for any $v \in \mathbb{R}$ such that this $\sup$ is finite : \[\Lambda(f) (v) = \log \sup_{x \in B_E(0,e^v)} \vert f(x) \vert,\]
and $\Lambda(f) (v) =+\infty$ otherwise.
We define  $NP(f)$ to be the Newton polygon of $f$, that is to say, the lower convex hull in $\mathbb{R}^2$ of the $(k, - \log \vert f_k \vert ),$ $k\geq 0.$ 
\end{deftn}

The following proposition is a classical consequence of convex analysis.

\begin{prop} \label{prop : conjugate Newton}
We define $v_\infty \in \mathbb{R} \cup \left\lbrace \infty \right\rbrace$ to be the limit of the slopes of $NP(f).$
We denote by $NP(f)^*$ the convex conjugate of $NP(f).$ $NP(f)^*$ is defined over $\left] -\infty,v_{\infty} \right[$ (at least)
\end{prop}

Then, $\Lambda(f)$ and $NP(f)$ are connected :

\begin{prop} \label{prop : connexion sup Newton}
For any $v \in \left]-\infty, v_\infty \right[,$ \[ \Lambda (f) (v) = NP(f)^*(v).\]
\end{prop}
\begin{proof}
Let $v \in \left] -\infty,v_{\infty} \right[,$ and let us assume that $v$ is not a slope of $NP(f).$
Let $u = NP(f)^*(v).$ Then the intersection of the line defined by $y=vx-u$ and $NP(f)$ is an extreme point of $NP(f),$ say $(n, - \log ( \vert f_n \vert ))$ for some $n \geq 0,$ and $u = vn + \log \vert f_n \vert.$

$v$ is not a slope of $NP(f)$ and by convexity of $NP(f)$, its difference quotients at $n$ are increasing, therefore, there exists some $c>0$ such that for any $m\geq 0,$ $vm-u \leq - \log \vert f_m \vert - c \vert n-m \vert.$ Since $vm-u=vm-vn-log \vert f_n \vert,$ we get $-vn-\log \vert f_n \vert + c \vert n-m \vert  \leq -vm-\log \vert f_m \vert .$

Now, let $x \in  B_E(0,e^v),$ and $m \geq 0.$ Definition of $\vert f_m \vert$ yields $ \vert f_m (x) \vert \leq \vert f_m \vert e^{vm}.$ Therefore \[\vert f_m (x) \vert \leq e^{-c \vert n-m \vert } \vert f_n \vert  e^{vn}< \vert f_n \vert e^{vn}.\]
Thus, the serie converges and by ultrametricity $ \vert f(x) \vert \leq \vert f_n \vert e^{vn},$ and $\Lambda (f)(v) \leq \log \left( \vert f_n \vert e^{vn} \right).$  

Now, definition of $\vert f_n \vert$  provides $(x_i)_{i\geq 0}$ in $B_E(0,e^v)$ such that $\lim \vert f_n (x_i) \vert= \vert f_n \vert e^{vn}.$ 
...
As a consequence, $\lim_i \vert f(x_i) \vert = \vert f_n \vert e^{vn},$ and  $\Lambda (f)(v) = \log ( \vert f_n \vert e^{vn}) = u= NP(f)^*(v).$
...
The following proposition will reveal crucial in order to minorate or majorate $NP(f),$  $NP(f)^*$ or $\Lambda(f),$ and is classical in convex analysis.
