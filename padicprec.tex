...
By describing how lattices transform under non-linear maps, it allows us to propagate
precision using differentials.  More specifically, it describes a class of first order lattices,
whose image under a map of Banach spaces is precisely described by applying the
...
In Section \ref{sec:tracking} we describe methods for tracking precision in practice.
...
accurate than the first one.  The precision loss inherent in an algorithm
is an important factor in analyzing its performance.  However,
since this precision loss depends heavily on the details of the problem
and the algorithm, we will not focus on such questions in this paper.

\subsection{Precision Types}

Using an arbitrary lattice to record the precision of an approximate element
has the benefit of allowing computations to proceed without unnecessary
precision loss using Lemma \ref{lem:main}.  However, while recording a
lattice exactly is possible it does require a lot of space.  For example,
the space required to store a lattice precision for a single $n \times n$
matrix with entries of size $O(p^N)$ is $O(Nn^4)$.  Conversely, the space
needed to record that every entry has precision $O(p^N)$ is just $O(\log N)$.
