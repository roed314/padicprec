...
respectively. Thus, we can consider the map $f : P \mapsto (A,B)$ 
...
map $P \mapsto A$ is then nothing but the mapping that follows 
...
map at $(f_1, \ldots, f_s)$ is continuous provided that:
\begin{itemize}
...
unique but they are on a small neighbourhood of $M$ if we fix the matrix 
$P$.
...
course, there exists a neighbourhood $\mathcal U$ of $M$ whose image is 
...
our chart to the map that takes $M$ to the unique matrix $N \in
