...
for $K = \Fp\lp t \rp$ we may choose $R = \Fp[t, t^{-1}]$ or $R = \Fp(t)$.
...
+ u_{n-2}^2}{u_n}$ exhibiting the Laurent phenomenon.
...
mainly because we are using the article \cite{vaccon:14b}, which deals only 
...
\ldots, g_t)$. It follows from \cite{vaccon:14b}*{Thm. 1.1} that this 
...
order (\emph{cf} \cite{vaccon:14b} for more detail).
\end{itemize}
