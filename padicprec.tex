...
\item Kedlaya \cite{kedlaya:01a}, Lauder \cite{lauder:04a} and many followers used $p$-adic cohomology to count points on hyperelliptic curves over finite fields;
...
all $p$-adic elements have constant \emph{absolute precision} $n$.
...
In both of these models, precision (absolute or relative) is constant across all elements.
Since some operations lose precision, it can be useful to attach a precision to each element.
Over the reals, such interval arithmetic is unwieldy, since arithmetic operations always increase
the lengths of the inputs.  As a consequence, most computations in the real numbers rely on
...
in computing the precision lattices.  We introduce precision types in Section \ref{ssec:types},
which allow a tradeoff between flexibility, space and time in computing with precision.
In Section \ref{ssec:SOMOS-solution}, we give
...
and Grassmannians.  Finally, Appendix \ref{sec:matrices} describes how to compute
the derivative of some common operations on matrices as an example, with an eye
...
%Recall that 
%an element $a + O(p^N)$ of $\Qp$ has \emph{absolute precision} $N$ if it 
%is well defined up to adding an unknown element of $p^N\Zp$.
...
The computation of $u_n$ involves a division by $u_{n-4}$ and hence, roughly speaking,
decreases the precision by a factor $p^{\val(u_{n-4})}$.  Hence the
...
From the latter formula and our assumption that $u_0$, $u_1$, 
$u_2$ and $u_3$ are units known up to precision $O(p^N)$, it follows that all $u_n$'s
are known with the same precision. Thus, the term $v_n$ that 
...
\begin{enumerate}
\item compute the polynomials $P_n$ using the recurrence in 
...
Let us first recall that a square matrix $M$
admits a LU factorization if it can be written as a product $LU$ where 
$L$ and $U$ are lower triangular and upper triangular respectively. 
...
discussed further in Appendix \ref{sec:matrices}.
...
accurate than the first one. On the other hand, the Cramer-type formulae
in the second algorithm yield a substantially longer running time than the first.
%In \cite{caruso:12a}, the author designs a new algorithm which combines 
%both advantages: efficiency and stability.
%The precision loss inherent in an algorithm is an important factor in 
%analyzing its performance.
%However, since this precision loss depends heavily on the details of the 
%problem and the algorithm, we will not focus on such questions in this 
%paper.
...
that $K$ contains a computable dense subring $R \subset K$ \cite{rabin:60a}.
This assumption holds for $K = \Qp$ and $K=\Fp\lp t \rp$ by setting $R = \Z[\frac1p]$ or $R = \Q$
in the case of $\Qp$ and $R = \Fp[t, t^{-1}]$ or $R = \Fp(t)$ in the case of $\Fp\lp t \rp$.
...
Note that $B_E(r)$ and $B^-_E(r)$ are both lattices.  We will also set $B_E(\infty) = B^-_E(\infty) = E$.
...
for scalars $\alpha_i \in K$ with $\alpha_i \to 0$ (according to the filter of cofinite subsets), and $\Vert x \Vert = \sup_{i \in I} \lvert \alpha_i \rvert$.
...
analysis: Lemma \ref{lem:main}.  This section develops the theory 
surrounding this result; we proceed to practical consequences in 
...
$v_0$ with
\[ 
\Vert f(v)-f(w)-f'(v_0) \left( v-w \right) \Vert 
...
This notion of differentiability is sometimes called \emph{strict differentiability};
it implies that the function $x \mapsto f'(x)$ is continuous on $U$. 
\end{rem}
...
$v_0$ if $v_0 + H \subset U$ and the following equality holds:
\begin{equation}
\label{eq:firstorder}
...
\begin{proof}
Without loss of generality, $v_0=0$ and $f(0)=0$. Since $f'(0)$ is surjective, the open mapping theorem provides a $C>0$ such that 
...
over many $W$ yields an upper bound on $f(v_0 + H)$.

\subsection{The case of locally analytic functions}
\label{ssec:locanalytic}

In this section we make the constant $\delta$ in Lemma \ref{lem:main} explicit,
under the additional assumption that $f$ is locally analytic.
We extend the definition of such functions from finite-dimensional
$K$-vector spaces \cite {schneider:11a}*{\S 6} to $K$-Banach spaces.

\begin{deftn}
\label{def:locanal}
Let $E$ and $F$ be $K$-Banach spaces. Let $U$ be an open subset of $E$
and let $x \in U$.
A function $f \col U \rightarrow F$ is said to be \emph{locally analytic} at 
$x$ if there exists an open subset $U_x \subset E$ and continuous $n$-linear
...
differentiable at $x$, with derivative given by $L_1$.
\end{rmk}
...
To such a series $f$, we attach the function $\Lambda(f) : \R \cup \{+\infty\} \to \R 
\cup \{+\infty\}$ defined by:
\[
\Lambda(f)(v) = \begin{cases} \log \left( \sup_{h \in B^-_E(e^v)} \Vert f(h) \Vert \right) & \mbox{if $f$ is defined on} B^-_E(e^v), \\
...
The function $\Lambda(f)$ we have just defined is closely 
related to the Newton polygon of $f$. Recall that the 
Newton polygon of $f$ is the convex hull in $\R^2$ of the points
$(n, - \log \lVert f_n \rVert )$ for $n \geq 0$, together 
...
order-reversing involution: $(\varphi^\star)^\star = \varphi$ and
$\varphi^\star \geq \psi^\star$ whenever $\varphi \leq \psi$. When
necessary, we extend $\varphi^\star$ to $\R \cup \{+\infty\}$ by
left continuity.
We refer to \cite{Rockafellar:97} for a complete exposition on Legendre transforms.
...
\begin{proof}
Note that the functions $\Lambda(f)$ and $\NP(f)^\star$ 
are both left continuous. It is then enough to prove that they
coincide expect possibly on the set of slopes of $\NP(f)$,
a dense subset of $\R$.

\smallskip

Let $v \in \R$, not a slope of $\NP(f)$. We assume first that 
...
\lVert f_n \rVert \cdot e^{vn} \le \lVert f_n \rVert \cdot e^{vn}.
\]
...
and the fact that $\vert K^\times \vert$ is dense in $\R$ ($K$ is algebraically closed)
...
$\lim_{i \to \infty} \Vert f_n (x_i) \Vert= \lVert f_n \rVert \cdot e^{vn}$. Since 
 $\Vert f_m (x_i) \Vert \leq e^{-c \cdot \vert n-m \vert} \cdot
\Vert f_n \Vert \cdot e^{vn}$ for all $m$ and $i$, we get $\Vert f_m (x_i) \Vert 
...
goes to $-\infty$ when $x$ goes to $+\infty$. By the
definition of $\NP(f)$, the expression $v n + \log \Vert f_n \Vert$
goes to infinity as $n$ grows. It is then enough to establish the
following claim:
\begin{equation}
\label{eq:claim}
\forall n \in \N, \quad 
...
sequence $\Vert f_m(x_n) \Vert$ goes to $0$ as $m$ goes to infinity.
...
$\lambda \in \OK$, the series defining $f(\lambda x_n)$ converges 
and
...
field $k$ of $K$.  Since $k$ is infinite, there must exist $\lambda \in 
\OK$ such that $\sum_{m \in I} \lambda^m f_m(x_n)$ does not vanish
...
\R \cup \{\pm \infty\}$ by
\[
\varphi_{\geq v}(x) = \inf_{y \geq 0} \, \big(\varphi(x + y) - v y \big).
\]
The function $\varphi_{\geq v}$ is the maximum among all functions $\varphi'$
with $\varphi' \leq \varphi$ and $x \mapsto \varphi'(x) - v x$ nondecreasing.
...
that $\varphi \leq \psi$ then $\varphi_{\geq v} \leq \psi_{\geq v}$.
...
We may now provide two sufficient conditions to effectively recognize 
first order lattices.
...
This inequality follows from Propositions \ref{prop:lambdaNP} and
...
program after the execution of the $i$-th step. Mathematically, we assume
that it is an 
...
variables after the step $i$. If all $g_i'(x)$ were surjective, this 
...
Therefore, modifying the intermediate value $x_i$ by an element
of $H'_i$ after the $i$-th step of the execution of {\tt f} leaves
the final result unchanged. In other words, it is
...
presented is then probably not much more expensive than the one-pass 
...
The study of this example actually suggests another solution to 
tackle the issue of non-surjectivity. Indeed, we remark that our $f$ above 
did not have a surjective differential simply because its codomain was too 
large: replacing the codomain with the $K$-vector space $S_n(K)$
of $n \times n$ symmetric matrices over $K$ makes the differential of $f$ surjective.
While the image of a general $f$ is rarely a sub vector 
space of $F$, it is often a sub-$K$-manifold of $F$. We can then use 
the results of Appendix \ref{sec:manifold} to study $f : U \to f(U)$, which
likely has surjective differential.

\medskip

\subsubsection*{Quick comparison with floating point arithmetic.}

The two strategies described above share some similarities with standard 
floating point arithmetic over the reals. In each setting, we begin
by choosing a large precision for all computations, and when we
encounter an unconstrained digit we choose it ``at random'' or using good heuristics. 
However, in the ultrametric setting, mild hypotheses allow us
to quantify the precision needed at each 
individual step in order to ensure a specified final precision.
...
curves \cite{kedlaya:01a}*{\S 4: Step 3} involves taking the characteristic polynomial of the matrix of Frobenius acting
...
\item If $E = \Qp \lb X \rb$, the \emph{Pollack-Stevens} precision type consists of lattices of the form $H_N := B_E((X^i),(p^{\min(i-N,0)}))$ \cite{pollack-stevens:11a}*{\S 1.5}.  These lattices are stable under certain Hecke operators, which is necessary for computing with overconvergent modular symbols.
\end{itemize}

Note that sometimes the precision of a final result can be computed
a priori (using the methods of Appendix \ref{sec:matrices} for example).
...
four-term inductive sequence defined by $u_n = \frac{u_{n-3} u_{n-1} 
+ u_{n-2}^2}{u_{n-4}}$ exhibiting the Laurent phenomenon.
We will focus on SOMOS sequences with values in $\Q_p$, and 
assume for simplicity that $u_0, u_1, u_2, u_3 \in \Z_p^\times$.
By the Laurent phenomenon, $u_n \in \Z_p$ for all $n$, and if $u_0, u_1, u_2, u_3$ 
...
\ref{line:lift}. Applying Lemma \ref{lem:main} and Proposition
...
\]
By the discussion above, this equation implies in particular that $f(a',b',c',d')$
belongs to $(u_i, u_{i+1}, u_{i+2}, u_{i+3}) + p^N \varphi_i(\Z_p^4)$. We
conclude by remarking that $(a,b,c,d) \equiv f(a',b',c',d') \pmod 
{p^\prec \Z_p^4}$ by construction and that $p^\prec \Z_p^4 \subset
p^N \varphi_i(\Z_p^4)$.
...
operation. In this appendix we compute the differential of various
common operations on matrices. Surprisingly, 
we observe that all differentials we will consider are rather easy to 
compute even if the underlying operation is quite involved.
...
Institut de recheche en math\'ematiques de Rennes (IRMAR) \\
