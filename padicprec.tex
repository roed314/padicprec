...
\begin{deftn} % Separated definitions?
Let $E$ be a $K$-Banach space.
A \emph{lattice} in $E$ is an open bounded sub-$\OK$-module of $E$.
\end{deftn}
...
% Lattice from Banach basis

\subsection{Approximate elements}

Suppose that $E$ is a $K$-Banach space with basis $(x_i)_{i \in I}$.  We say that an element
$x \in E$ is \emph{exact} if there is a finite subset $J \subseteq I$ and scalars $\alpha_j \in R$ with
\begin{equation} \label{eq:exact_elt}
x = \sum_{j \in J} \alpha_j x_j.
\end{equation}
The set of exact elements is dense in $E$.

An approximate element is a pair $(x, H)$ where $x \in E$ is an exact element
and $H$ is a lattice in $E$.  Such a pair represents an undetermined element of the coset
...
Note that using an arbitrary lattice to represent the precision of an approximate element
