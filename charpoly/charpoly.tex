...
$\OK$ its ring of integers and $\pi$ a uniformizer. \todo{$\softO$, $\omega$, scalar matrices and $I_n$, $\sigma_1(M), \dots, \sigma_n(M)$}
...
known at precision at least $O(\pi^{m-\cond(M)})$, with coefficients in $O_K$
and determinant in $O_K^\times$ realizing an Smith normal form of $M.$
...
entries lie in the CDVF $K((X))$.  In this way, we compute $\com(X-H)$
