...
Let $\com(M)$ denote the comatrix of $M$; when $M \in \GL_n(K)$ then $\com(M) = \det(M) M^{-1}$.  Then $d\chi_M$ is given
by
\[
d\chi_M: dM \mapsto \tr(\com(X-M) \cdot dM).
\]
%Moreover, $d\chi(M)$ will be surjective exactly when 
...
and we have $P(M) \cdot C \equiv  P(X) \cdot C \pmod{\chi_M(X)}$.}
\end{proof}
