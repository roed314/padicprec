...
Thanks to the application the framework of differential precision of \cite{caruso-roe-vaccon:14a} in \cite{caruso-roe-vaccon:15a},
we known that
the precision on the characteristic polynomial
$\chi_M$ of a matrix $M \in M_n(\mathbb{Q}_p)$
is determined by the comatrix $com(X I_n -M).$
In this article, we provide:
\begin{enumerate}
\item A factorization of $com(X I_n -M)$ (when $M$ has a cyclic vector).
\item A simple criterion to decide whether there can be a gain in precision
on $\chi_M$ when $M \in M_n(\mathbb{Z}_p).$
\item A $\softO (n^3)$ algorithm with operations in $O_K$
