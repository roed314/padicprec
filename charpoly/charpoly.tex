...
of $X^{n-k}$ in $\chi_M$ will be $\sum_{i=1}^k \alpha_i$, and if $M$ is known
with precision $O(p^m)$ then $\chi_M$ will be known with some precision
larger than $O(p^m)$ (see Proposition \ref{prop:mainlem}).
...
We shall see moreover in Subsection \ref{ssec:eigenvalues} that interesting
