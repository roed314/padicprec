...
\item \done{The notion of lattice (in the tangent space) has been defined.}
...
We recall some of the definitions and results of 
\cite{caruso-roe-vaccon:14a} as a foundation for our discussion of the 
precision for the characteristic polynomial of a matrix. We will be 
concerned with two spaces in what follows: the space $M_n(K)$ of $n 
\times n$ matrices with entries in $K$ and the space $K_n[X]$ of monic 
degree $n$ polynomials over $K$. We define as well $K_{<n}[X]$ (resp. 
${\OK}_{<n}[X]$) as the space of polynomials over $K$ (resp. over $\OK$) 
of degree strictly less than $n$.

There are several choices for modeling precision on $p$-adic matrices. 
The first one consists in representing each entry of a matrix as a 
quantity of the form $a_{i,j} + O(\pi^{N_{i,j}})$. The integer $N_{i,j}$ 
is called the \emph{absolute precision} and may depend on $(i,j)$. The 
way of representing precision will be called \emph{jagged} in the 
sequel. When the $N_{i,j}$ are all equal (\emph{i.e.} when each entry 
is given at the same precision), we say that the precision datum is 
\emph{flat}.
A more flexible way of modeling precision consists in using lattices. We 
recall that a lattice in a $M_n(K)$ the image of $M_n(\OK)$ under a 
$\Qp$-linear transformation of $M_n(K)$. Since $\OK$ appears as the 
closed unit ball of $K$ (for the $p$-adic norm), cosets of the form
$M + H$ (where $M$ lies in $M_n(K)$ and $H$ is a lattice) are special
neighborhood of $M$; we will use them for modeling precision on $M$. 
Given a family of integers $\famN = (N_{i,j})_{i,j}$ the lattice:
$$H_{\famN} = 
\big\{ \, M = (a_{i,j})_{i,j} \in M_n(\Qp) \,\, | \,\, \forall i,\!j\!:
\pi^{N_{i,j}} \text{ divides } a_{i,j} \, \big\}$$
corresponds to the jagged precision introduced above.

Jagged and flat precisions are defining similarly for monic polynomials 
(taking care that no precision datum is needed for the leading 
coefficient which is supposed to be exactly~$1$). Simiarly, the notion 
of \emph{lattice precision} extends to polynomials. The underlying 
...
differentiable maps; this is the content of 
\cite{caruso-roe-vaccon:14a}*{Lem. 3.4}. Write $\chi : M_n(K) \to 
K_n[X]$ for the characteristic polynomial map and $d \chi_M$ for its 
differential at $M$. Thanks to 
Recall from \citelist{\cite{caruso-roe-vaccon:14a}*{Appendix B} 
\cite{caruso-roe-vaccon:15a}*{\S 3.3}} that $d\chi_M$ is given by
...
\end{array}
\end{equation}
where $\adj(M)$ denotes the adjugate\footnote{In 
\citelist{\cite{caruso-roe-vaccon:14a} \cite{caruso-roe-vaccon:15a}} the 
adjugate was referred to using the non-standard term \emph{comatrix}} of 
$M$ (when $M \in \GL_n(K)$, we have $\adj(M) = \det(M) M^{-1}$) and 
$d\chi_M$ the differential at $M$. In this setting, 
\cite{caruso-roe-vaccon:14a}*{Lem. 3.4} says that
\begin{equation}
\label{eq:mainlem}
\chi(M + H) = \chi(M) + d\chi_M(H)
\end{equation}
whenever $H$ is a sufficiently ``small'' and ``regular'' lattice.
More precisely, let $\sigma_1(M), \dots, \sigma_n(M)$ denote the 
...
\]
Then, for all integers $N, M \geq 0$ with $M + \nu < 2N$
any lattice $H$ such that $M_n(\pi^N \OK) \subset H \subset
M_n(\pi^M \OK)$
...
the computation of its derivative $d\chi$, using 
\cite{caruso-roe-vaccon:14a}*{Lem. 3.4}.
...
\noindent
{\bf About surjectivity.}
The surjectivity of $d \chi_M$ appeared as an assumption in 
Proposition~\ref{prop:mainlem}. The next Proposition make it
entirely explicit.
