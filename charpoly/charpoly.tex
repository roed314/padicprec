...
$K_n[X]$ of monic degree $n$ polynomials over $K$.  One cannot represent
matrices $M \in M_n(K)$ exactly, but instead must specify an approximation
together with a measure of the approximation's accuracy.  Recall that a
\emph{lattice} in $M_n(K)$ is an open $\OK$-submodule of $M_n(K)$:
the $\OK$-span of $n^2$ basis vectors \cite{caruso-roe-vaccon:14a}*{\S 2.2}.
We record the precision of $M$ with lattice $H$: the matrix $M$ is understood
as some matrix of the form $M + h$, where $h \in H$.

One problem with representing precision in this way is that storing $H$
requires $n^2$ basis vectors, each with $n^2$ entries.  We can shrink the
space required by working only with certain classes of lattices.  When
using \emph{jagged} precision, the basis matrices for $H$ can only have
one nonzero entry; this corresponds to specifying a precision for each
entry of the matrix separately.  Going further, $M$ is said to have \emph{flat}
precision $O(p^N)$ if every entry has precision $O(p^N)$, or equivalently,
if $H = p^N M_n(\OK)$.  Similarly, precision for
monic polynomials can be specified by giving a lattice in $K_{<n}[X]$,
or specifying precision coefficient-wise in the interest of simplicity.
...
When $d\chi_M$ is surjective and $H$ is small enough, we may determine the precision of $\chi(M)$ precisely.
Write $V$ for $M_n(K)$, $W$ for $K_n[X]$, 
$B^-_V(r)$ (resp. $B_V(r)$) for the open (resp. closed) ball of radius $r$ in $V$, and let
$\sigma_1(M), \dots, \sigma_n(M)$ denote the elementary divisors of $M$.

\begin{prop} \label{prop:mainlem}
Let $M \in M_n(K)$ and suppose that $d\chi_M$ is surjective.  Let 
\[
