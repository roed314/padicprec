...
\todo{Explain here why computing characteric polynomial is so important}
...
\todo{Many \emph{ad-hoc} for Kedlaya-like counting point algorithms.
Our previous paper. What else?}
...
\noindent
{\bf Notations.}
Throughout the paper, $K$ will refer to a complete, discrete valuation 
field, $\val : K \twoheadrightarrow \Z \cup \{+\infty\}$ to its 
valuation, $\OK$ its ring of integers and $\pi$ a uniformizer.
...
\begin{cor}
Suppose that $M \in \GL_n(\OK)$ is known with precision $O(p^N)$.
Then the characteristic polynomial of $M$ has precision larger than $O(p^N)$
...
n \geq 1$$
and $a$ is a random integer in the range $[0, p^N)$ with 
taken with uniform distribution
