...
precision of the coefficients of $\chi_M$ and of their eigenvalues.
...
Let $\adj(M)$ denote the adjugate\footnote{In \citelist{\cite{caruso-roe-vaccon:14a} \cite{caruso-roe-vaccon:15a}} the adjugate was referred to as the \emph{comatrix}} of $M$ (when $M \in \GL_n(K)$,
...
$P$ is exact 
%over finite extensions of $\mathbb{Q}_p$ and $k\llp X\rrp$, 
...
%the lift(in $\mathbb{Z}, $  $\Fq[X],$ $\Q[X]$ or adequate extension) of 
...
%Over other fields, we can not lift, but the computations are still valid.
The rest is clear.
\end{proof}
...
%It is $-2\cond(M)$ for $M^{-2}$ and it is then clear that 
...
%Indeed, one can use multilinearity of the determinant on $X^d \det(F(1/X))$ to prove this result.
