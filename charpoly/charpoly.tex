...
Let $\chi : M_n(K) \to K_n[X]$ be the characteristic polynomial map.  For a matrix $M \in M_n(K)$
we identify the tangent space $V$ at $M$ with $M_n(K)$ itself, and the tangent space $W$ at $\chi(M)$ with
the space $K_{<n}[X]$ of polynomials of degree less than $n$.  Write $B^-_V(r)$ (resp. $B_V(r)$) for the open (resp. closed)
ball of radius $r$ in $V$.  
Our analysis of the $\chi$ begins with the following result
\citelist{\cite{caruso-roe-vaccon:14a}*{Lem. 3.4} \cite{caruso-roe-vaccon:15a}*{Prop. 2.2}}.

\begin{prop} \label{prop:mainlem}
Suppose that $M \in M_n(K)$ and that $d\chi(M): V \to W$ is surjective.
Let $C$ be a positive real number such that
$B_W(1) \subset d\chi(M)(B_V(C))$.  Then, for all $\rho \in (0, 1]$ and all
...
In light of Proposition \ref{prop:mainlem}, we recall the computation of $d\chi$
\citelist{\cite{caruso-roe-vaccon:14a}*{Appendix B} \cite{caruso-roe-vaccon:15a}*{\S 3.3}}.
Let $\com(M)$ denote the comatrix of $M$; when $M \in \GL_n(K)$ then $\com(M) = \det(M) M^{-1}$.  Then $d\chi(M)$ is given
by
\[
d\chi(M): dM \mapsto \tr(\com(X-M) \cdot dM).
\]
Moreover, $d\chi(M)$ will be surjective exactly when the Jordan blocks of $M$ all have distinct generalized eigenvalues.   
...
Equality of lattices holds iff it does mod p, so need surjectivity modulo p.  Surjectivity of differential
...
\section{Optimal precision\\via Hessenberg form}
...
\section{Optimal precision\\via Frobenius form}
