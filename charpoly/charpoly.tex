...
stable algorithm. \todo{reference a place with more detail.}
...
%\todo{Many ad-hoc for Kedlaya-like point counting algorithms.  Our previous papers.  What else?}
...
refer to fast algorithm (using divisions), while others
...
how to achieve this optimal precision.
...
$\OK$ its ring of integers and $\pi$ a uniformizer. \todo{$\softO$, $\omega$, scalar matrices and $I_n$}
...
Then the characteristic polynomial of $M$ has precision lattice strictly contained in $O(p^m)$
...
If $M \in M_n(O_K)$, then only division by invertible
elements of $O_K$ will occur.
...
It is $-2\cond(M)$ for $M^{-2}$ and it is then clear that $-2\cond(M)$
...
we obtain $M^{-1}=Q^{-1} \Delta^{-1} P^{-1}$ known at precision at least $O(\pi^{N-2\cond(M)}),$
...
inverse in a CDVF, we hope to get reasonable behavior
...
$U_{i,i}=1+XL_{i,i} \mod X^{n+1}$ (with $L_{i,i} \in O_K[X]$). 
...
\end{proof}
\begin{rem}
If $M \in M_n(K)$ does not have coefficients in $O_K,$
then, by an appropriate scaling, we can still use
Algorithms 1 and 2 to get $\chi_M$
in $\softO (n^3)$ operations in $O_K$ at some shift
of the initial precision.
\end{rem}
...
In this Subsection, we are interested in a special
case of the computation of the comatrix of $X-M,$
when $\chi_M$ is square-free (which is the 
generic case).
We observe that it is possible to combine
the result of Proposition \ref{prop:shortcom}
with Algorithm 2 to actually compute
the comatrix of $X-M.$
It leads to a $\softO (n^3)$ algorithm
where the only divisions are for
gcd and modular inverse computations.
...
\begin{theo}
For $M \in M_n(O_K)$ such that $discr( \chi_M) \neq 0,$
Algorithm 3 compute 
$\com (X-M) \mod (\chi(M))$ in average complexity $\softO (n^3)$
operations in $K$ at initial precision.
The only divisions are in steps 4 and 5.
\end{theo}
\begin{proof}
...
on Step 4.
Now, it is clear that the only divisions happening are on steps 4 and 5.
...
get coefficients in $O_K$ and apply the previous algorithm.
\end{rem}
\begin{rem}
We refer to \cite{caruso:15a} for the handling
of the precision on gcd and modular inverse computations.
...
$\softO(n^\omega)$ operations in $K$ using the first algorithm
...
In the previous Sections, \ref{sec:diffHess} and \ref{sec:diffHess},
...
large precision and then use 
and then to perform a computation of the characteristic polynomial 
