...
It is $-2cond(M)$ for $M^{-2}$ 2 and it is then clear that $-2cond(M)$
...
known at precision at least $O(\pi^{N-cond(M)})$ with coefficients in \todo{integral ring}
and determinant in \todo{integer ring invertible} realizing an SNF form of $M.$
...
Firstly, we need a lemma to relate comatrices for
similar matrices:
...
The first ingredients are reciprocal polynomials.
We extend their definition to matrices of polynomials.
\begin{deftn}
...
is of positive dimension.
As any the proof of \ref{prop:factor_com}, we can work modulo $X-\lambda$
for $\lambda$ a root of $\chi (M)$ (in an algebraic closuer)
...
of hyperplane, and hence, is of positive dimension.
...
precision is known, it is possible to possible lift arbitrarily
