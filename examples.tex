...
around $0$. The composition $h = g \circ
...
\]
where the supremum is taken over nonnegative integers $m$
and sequences $(n_i)_{1 \leq i \leq m}$ of nonnegative integers
with $n_1 + \cdots + n_m = r$.
\end{prop}

Multinomial coefficients will play an important role in the proof, so we recall their definition. 
Given positive integers $s, k_1, \ldots, k_\ell$ with $s \geq k_1 + \cdots + k_\ell$, set
\[
\binom s {k_1 \,\, k_2 \,\, \cdots \,\, k_\ell} =
  \frac{s!}{k_1!\: k_2! \cdots k_\ell! \: (s{-}k_1{-}\cdots{-}k_\ell)!}.
\]
These numbers are all positive integers.
...
\end{align}
where the second sum runs over
...
\begin{proof}
Just remark that $y = 2(x + \alpha + \mu) + y_0$ is a equation of the 
dashed blue line.
\end{proof}
