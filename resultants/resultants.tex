...
4.2]{real}, \cite[\S 3.3]{cohen} and \cite[\S 4.1]{winkler}.
...
in this case. In other words, still assuming that all principal 
subresultants do not vanish, the sequence of subresultants obeys to 
the recurrence:
\begin{align}
\label{eq:recRj}
...
Algorithm~\ref{algo:subres}, which uses the primitive 
...
standard Euclide algorithm. The main interest of using 
Algorithm~\ref{algo:subres} is to avoid denominators, \emph{i.e.} we
always work over $\A$ instead $K$.
...
and $(V_j)_j$ satisfy the recurrences \eqref{eq:recRj}--\eqref{eq:recVj}.
...
$(R_j, R_{j-1})$ and we are done. The last equality is proved in a 
similar fashion using Eq.~\eqref{eq:recUj}.
\end{proof}
