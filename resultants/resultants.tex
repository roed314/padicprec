...
% parler notamment de précision absolue et de précision relative
% expliquer comment la précision se comporte vis-à-vis des opérations
% traiter un exemple de calcul de pgcd avec l'algorithme d'Euclide
...
% Expliquer qu'on ne donne qu'un résultat, pas un algorithme
...
\begin{rem}
The Proposition \ref{prop:prem} gives a formula for \emph{all} subresultants. We
...
$(S_i)$ then stops at $i = d_B$. Moreover, the $\varepsilon_i$'s and the 
$c_i$'s are now all ``trivial'' since we have $\varepsilon_i = 1$ and $c_i
...
\todo{
Expliquer que les pertes avec l'algorithme des pseudo-restes
sont les mêmes que ceux avec l'algorithme d'Euclide.
}
...
In this subsection, we briefly report on the results of \cite{padicprec}. In 
\emph{loc. cit.}, the authors draw the lines of a general framework to handle a 
sharp (often optimal) track of ultrametric precision.
\todo{Give more details here.}
...
\todo{Add a subsection to explain the stabilization process in
general.}
