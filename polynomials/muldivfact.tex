...
\noindent
{\bf Newton precision.} \todo{Write a one-line introduction.}

\begin{deftn}
A \emph{Newton function of degree $n$} is a convex function 
$f : [0,n] \to \R$ which is piecewise affine and whose epigraph
$\Epi(f)$ have integral extremal points.
\end{deftn}

\begin{rem}
The datum of $f$ is equivalent to that of $\Epi(f)$ and they can
easily represented and manipulated on a computer.
\end{rem}
...
given a Newton function $f$ of degree $n$, we agree that a polynomial 
of degree at most $n$ is given at precision $O(f)$ when, for all $i$,
its $i$-th coefficient is given at precision $O\big(\pi^{\lceil f(i)
\rceil}\big)$ (where $\lceil \cdot \rceil$ is the ceiling function).
In the sequel, we shall write
$O(f) = \sum_{i=0}^n O\big(\pi^{\lceil f(i) \rceil}\big) \cdot X^i$
and use the notation $\sum_{i=0}^n a_i X^i + O(f)$
to refer to a polynomial given at precision $O(f)$.

We observe in particular that Newton precision data are particular
instances of jagged precision. The main interest of using them is the
efficiency.
