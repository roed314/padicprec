...
discrete valuation fields (\emph{e.g.} $\Q_p$ or $k((t))$ for a 
field $k$), namely Euclidean division and factorization. We consider 
notably the question of computing slope factorizations and design a new 
simple and efficient algorithm based on Newton iteration to solve it.
One of its main feature consists in avoiding working over an extension 
of the base field.
We constantly pay particular attention to stability: we design several
precision models, compare them and always discuss optimality.
\end{abstract}
...
In the sequel, we use the notation $A \div B$ (resp $A \mod B$)
...
& x & \mapsto & \inf_{h \geq 0} \psi(x+d+h) - \lambda h.
\end{array}$$
\end{deftn}
...
the remainder of the Euclidean division of $dX = dA - B_\app dQ$ by $B_\app$.
