...
Some formulae :
\begin{align*}
B_i-B_{i+1} &= R_i B_{i+1} \div A_i, \\
S_i-S_{i+1} &= R_i B_{i+1} \div A_i, \\
S_{i} &= (B_i R_i + (1-V_i B_i) S_{i-1}+(1-V_i B_i)(S_i-S_{i-1})) \% A_{i}, \\
V_{i+1}-V_i &= (V_i (1-V_i B_i)+V_i^2(B_i-B_{i+1})) \% A_{i}, \\
1-Q_i &= (1-V_i B_i)+((R_i-V_i S_i) \div A_i), \\
R_{i+1} &= ((V_{i+1}-V_i)S_{i+1}+(1-Q_i)R_i) \% A_{i+1}, \\
1-V_{i+1}B_{i+1} &= ((1-V_i B_i)+V_i (B_i-B_{i+1}))^2.
\end{align*}
\end{lem}
...
\paragraph{Initial Newton polygons}
...
\paragraph{Unchanged Newton polygons}
...
\paragraph{Newton polygons inside the loop}
