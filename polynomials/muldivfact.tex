...
in such algorithms \cite{pauli:10a}*{Section~2}.  Note that our algorithm lifts
...
has a break at $i=d$.  Denote by $A \div B$ and $A \mod B$ the quotient and remainder
in the Euclidean division of $A$ by $B$. Set $A_0 = \sum_{i=0}^d a_i X^i$, $V_0 = 1$ and
\begin{align*}
...
slope factorization \cite{chistov:90a}*{Eq. (3)}, ours does not require working
with fractional exponents.
...
If $P(X) = \sum_{i=0}^n a_n X^n \in K[X]$, we define its Newton
...
exponents \cite{chistov:90a}*{Eq. (3)}. We introduce here a variant of this iteration which does not 
...
Let $P \in K[X]$ have degree $n$ and $d$ be the abscissa of an extremal point of 
...
precision up to degree $d{-}1$ and $n{-}d{-}1$ (resp).
...
\noindent{\bf Input:} a monic polynomial $P + O(\varphi_P) \in K[X]$ of degree $n$,
...
that the above algorithm is correct and 
stable. Using the standard notation $\softO(\cdot)$ for
hiding logarithmic factors, our algorithm performs at most $\softO(n)$ 
...
raised above: the precision of the pair $A^{(1)}$ is the lattice 
...
precision of $A^{(1)}$ (jagged) with the estimation given using 
