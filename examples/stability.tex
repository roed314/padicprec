...
discrete valuation field of characteristic $0$}, \todo{why characteristic 0?} that is either a finite 
...
expression equals $0$ if $p = 0$. \todo{which expression; the one defining $\alpha$?}
...
The blue \todo{do we have color?} plain line represents the graph of the function $\Lambda_f$. A 
...
$\pi^N$ and has diagonal entries $a_1, \ldots, a_n$ of valuations 
...
For any precision lattice $H$, the precision of the characteristic polynomial will thus be the lattice in $K_{<n}[X]$ generated by the polynomials $\tr(\com(X - M) \cdot dM)$, as $dM$ ranges over a set of generators for $H$.  In the standard case that the precision lattice for $M$ is just $H = \pi^N M_{n,n}(\O_K)$, we can further analyze $\charp(H)$.
...
\begin{prop}
Suppose a matrix $M$ is known with precision $H = \pi^N M_{n,n}(\O_K)$.  Then the precision of the characteristic polynomial of $M$ is contained within $S_N$ applied to the lattice of the precision polygon of $M$.
\end{prop}
\begin{proof}
