...
lattice. We now introduce another polygon that will be bounded by 
between these two, and will provide an estimate for $\mathcal{E}$.
...
Proposition \ref{prop:precision}, the precision polygon determines the 
best precision loses we can hope if we are encoding precision on 
polynomials by polygons.

It turns out that the precision polygon is related to Hodge and Newton 
polygon. If a polygon $P$ has vertices $(x_i, y_i)$, we let $T_n(P)$ be 
...
using the Hodge polygon loses precision but not so much. Nonetheless, as 
already illustrated for matrix multiplication, this loses of precision 
may accumulate and become significant if we need to compute many 
characteristic polynomials. In the case of a single characteristic 
polynomial, we note moreover that if the $\sigma_i(M)$'s are known to be 
