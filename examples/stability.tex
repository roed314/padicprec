...
(A,B) & \mapsto & AB.
\end{array}$$
According to Proposition \ref{}, the behaviour of the precision when 
...
to apply Proposition \ref{}, we then need to compute the image 
...
a^{(m)} b^{(m)}$ significant digits (at least if $N$ is large enough).
However these digits are not --- 
...
``diffused'' gains of precision. Of course, this strongly depends on the 
problem we are interested in.
Here is nevertheless a simple --- and hopefully persuasive --- example 
illustrating that we may gain a lot by considering lattice precision
data. \todo{Write the example.}
...
\ref{}. Therefore, from now on, we suppose that $M$ is invertible.
