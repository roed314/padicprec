...
We also analyze determinants and characteristic polynomials using these differential methods.
We supplement these observations with numerical experiments.
\end{abstract}
...
More precisely, if $F$ is equipped with the norm $\Vert \cdot \Vert_F$, 
we recommand endowing it with the second norm defined by
...
For any precision lattice $H$, the precision of the characteristic polynomial will thus be the lattice in $K_{<n}[X]$ generated by the polynomials $\tr(\com(X - M) \cdot dM)$, as $dM$ ranges over a set of generators for $H$.  In the standard case that the precision lattice for $M$ is just $H = \pi^k M_{n,n}(\O_K)$, we can further analyze $\charp(H)$.

Recall that the \emph{Newton polygon} of a polynomial $\sum_{i} a_i X^i$ is the lower convex hull of the points $(i, \val(a_i))$ and the \emph{Newton polygon} of a matrix is the Newton polygon of its characteristic polynomial.  If $m_1 \le \cdots \le m_n$ are the valuations of the diagonal entries of $\Delta_M$ in a Smith factorization $U_M \cdot \Delta_M \cdot V_M$ of $M$ as in the previous section, the \emph{Hodge polygon} of $M$ is the lower convex hull of the points $(i, \sum_{j = i+1}^n m_i)$.

\begin{deftn}
The \emph{precision polygon} of $M$ is the lower convex hull of the Newton polygons of the entries of $\com(X - M)$.  
\end{deftn}

Any such polygon $P$ naturally yields a lattice in $K_{<n}[X]$, consisting of polynomials whose Newton polygons lie above $P$.  The lattice associated to $P$ is generated by monomials $a_iX^i$, where $\val(a_i)$ is the ceiling of the height of $P$ at $i$.

If a polygon $P$ has vertices $(x_i, y_i)$, we $T_n(P)$ to be the translated polygon with vertices $(x_i - n, y_i)$ and $S_n(P)$ be the translated polygon with vertices $(x_i, y_i + n)$.

\begin{prop}
Suppose a matrix $M$ is known with precision $H = \pi^k M_{n,n}(\O_K)$.  Then the precision of the characteristic polynomial of $M$ is contained within $S_k$ applied to the lattice of the precision polygon of $M$.
\end{prop}
\begin{proof}
The Newton polygon of each entry of $\com(X - M)$ lies above the precision polygon by definition.  Since these generate $\charp(M)(H)$, the result follows.
\end{proof}


\begin{prop} \label{prop:prec_hodge}
The precision polygon of $M$ lies above $T_1$ applied to the Hodge polygon of $M$.
\end{prop}

\begin{proof}
\todo{Add this proof from Xavier's e-mail}
\end{proof}

For many matrices, the precision polygon equals the Hodge polygon.  For integer matrices with entries uniformly chosen between $0$ and $1024$, the $2$-adic precision polygon is equal to the Hodge polygon in $99.5\%$ of cases in dimension $4$, down to $99.1\%$ in dimension $8$.  \todo{p=3?}  Moreover, experiments support the following conjectures.

\begin{conj}
\begin{enumerate}
\item The precision polygon is equal to the Hodge polygon if the determinant of $M$ has valuation $0$ or $1$, or if $M$ is $3 \times 3$.
\item The height at $0$ of the precision polygon is equal to the height at $1$ of the Hodge polygon.
\item The precision polygon lies below $T_1$ applied to the Newton polygon.
\end{enumerate}
\end{conj}
Empirically, the precision polynomial seems most likely to differ from the Hodge polygon at $1$, corresponding to the precision of the linear term of the characteristic polynomial.

Of course, the precision polygon does necessarily agree with the full precision of the characteristic polynomial of $M$.  In $11\%$ of cases in dimension $3$, up to $15\%$ of cases in dimension $8$, the lattice associated to the precision polygon contains the actual precision lattice with index greater than $1$.  This percentage increases as the valuation of the determinant increases, reaching $34\%$ in dimension $9$ for matrices constrained to have determinant with $2$-adic valuation $12$.

Moreover, one can construct examples with arbitrarily large extra lattice precision.  If $M$ has elementary divisors with valuations $m_1 \le \cdots \le m_n$, Proposition \ref{prop:prec_hodge} implies that the precision lattice of the characteristic polynomial of $M$ is contained within $\O_{K,n}[X]$ with index at least $\sum_{i=1}^{n-1} m_i$.  The precision lattice of $1 + M$ is obtained from the precision lattice of $M$ via the transformation $X \mapsto X + 1$, but the precision polygon is now flat with height $0$.

\subsection{LU factorization}

\todo{Improve this Subsection.}

In this Subsection, we apply our framework to the case of $LU$ factorization. The computation of the differential of this factorization has already been done in \cite[Appendix B]{caruso-roe-vaccon:14a}. It remains to determine under which condition Proposition \ref{prop:precision} applies.


\begin{deftn}
Let $n \in \mathbb{Z}_{\geq 0}$ and  $O_n \subset K^{n \times n}$ be the set of the matrices such that all principal minors are non-vanishing. Let $U_n$ (resp. $L_n$)  be the $K$-vector spaces of the upper-triangular (resp. the strictly lower-triangular) matrices in $K^{n \times n}.$ Let $L_n^u \subset K^{n \times n} $ be the set of lower-triangular matrices with $1$'s one the diagonal. We define $D : O_n \rightarrow L_n^u \times U_n$ to be the mapping sending a matrix $M$ to $(L,M)$ such that $LU$ is the LU factorization of $M.$
\end{deftn}

This example has been previously studied by X.Caruso in \cite{caruso:12a}, but without using differential precision.
We show here that the framework of Subsection \ref{ssec:boundLambdaf} applies directly.

\subsubsection*{Computation of the differential of $D$} 
We begin with some notations:
\begin{deftn}
We denote by $\textrm{low}$ (resp. $\textrm{up}$) the canonical projection $K^{n \times n} \rightarrow L_n$ (resp. $K^{n \times n} \rightarrow U_n$).
\end{deftn}

Then, an explicit formula for $D'$ has been computed in \cite[Appendix B]{caruso-roe-vaccon:14a} :
\begin{prop}
Let $(M,L,U) \in O_n \times L_n^u \times U_n$ be such that $M=LU.$ Then, 

\begin{tabular}{cccc}
$D'(M) :$ &  $K^{n \times n}$ & $\rightarrow$ & $L_n \times U_n$ \\ 
 & $\mathrm{d}M$ & $ \mapsto$ & $(L \textrm{low}(L^{*-1} \mathrm{d}M U^{-1}), \textrm{up}(L^{*-1} \mathrm{dM} U^{-1}) U).$ \\ 
\end{tabular} 
\end{prop}

A right inverse of $D'(M)$ can be made explicit:
\begin{rem} \label{rem:rightinverseLU}
Let \begin{tabular}{cccc}
$E(M) :$ & $L_n \times U_n$  & $\rightarrow$ &  $K^{n \times n}$\\ 
 & $(A,B)$ & $ \mapsto$ & $AU+LB$. \\ 
\end{tabular} 

Then $D'(M)  \circ E(M) = Id.$
\end{rem} 

\subsubsection*{Applying Subsection \ref{ssec:boundLambdaf}}

In order to apply Subsection \ref{ssec:boundLambdaf}, $f$ will be a translation of $D,$ so as to ensure $g(0) \neq 0.$ Namely,  let $(M_0,L_{0},U_{0}) \in O_n \times L_n^u \times U_n$ be such that $M_0 = L_{0}U_{0},$ we define

\begin{tabular}{cccc}
$f :$ &  $K^{n \times n}$ & $\rightarrow$ & $L_n \times U_n$ \\ 
 & $M$ & $ \mapsto$ & $(L_{M+M_0}-L_{0}, U_{M+M_0}-U_{0}).$ \\ 
\end{tabular} 

Then, $f(0)=0$ and \[f'(M) \cdot \mathrm{d}M = (L_{M+M_0} \textrm{low}(L_{M+M_0}^{-1} \mathrm{d}M U_{M+M_0}^{-1}), \textrm{up}(L_{M+M_0}^{-1} \mathrm{dM} U_{M+M_0}^{-1}) U).\]

We now proceed with the three steps of the framework of Subsection \ref{ssec:boundLambdaf}.

\textbf{The $C$ constant:}

\begin{lem}
For $C = \max (\Vert U_{0} \Vert,\Vert L_{0} \Vert ),$ $B_{K^{n \times n}}(1) \subset f'(0) \cdot B_{K^{n \times n}}(C).$ 
\end{lem}
\begin{proof}
Apply Remark \ref{rem:rightinverseLU}.
\end{proof}

\textbf{The differential equation:}

\begin{lem} Let $g=g_{M_0}$ be defined by:
\begin{align*}
g(A,B) \cdot X =& ( (L_0^*+A) \textrm{low} \left( (L_0^*+A)^{-1}X(U_0+B)^{-1} \right), \\
& \: \textrm{up} \left( (L_0^*+A)^{-1}X(U_0+B)^{-1} \right) (U_0+B) ).\\
\end{align*}
Then, \[f' = g \circ f.\]
\end{lem}

\textbf{Determining $\mu$ and $\nu$}

Let $\vert \: \vert $ be an ultrametric norm on $K$ and $\Vert \: \Vert$ be the corresponding subordinate matrix norm. To determine $\mu$ and $\nu,$ we first use the following lemma

\begin{lem}
Let $(U,V) \in K^{n \times n} \times K^{n \times n}.$ If $\Vert V \Vert < \Vert U \Vert$ then $\Vert U+V \Vert = \Vert U \Vert.$

If $U \in GL_n(K)$ and $\Vert U \Vert < \Vert U^{-1} \Vert^{-1},$ then $\Vert (U+V)^{-1} \Vert = \Vert U^{-1} \Vert.$

Finally $\Vert U^{-1} \Vert^{-1} \leq \Vert U \Vert.$
\end{lem}

This leads to the following upper-bound on the norm of $g(A,B)$.

\begin{prop}
Let $(A,B) \in L_n \times U_n.$ be such that $\Vert A \Vert < \Vert L_0^{*-1} \Vert $ and $\Vert B \Vert <\Vert U_0^{*-1} \Vert. $ Then 
\[ \Vert g(A,B) \cdot X \Vert \leq \max \left( \Vert L_0^* \Vert \Vert L_0^{*-1} \Vert  \Vert U_0^{*-1} \Vert, \Vert U_0^* \Vert \Vert U_0^{*-1} \Vert  \Vert L_0^{*-1} \Vert \right) \times \Vert X \Vert . \]
\end{prop}

\begin{prop}
For $f$ and $g$ as before, we can take $\nu = \min ( \Vert L_0^{-1} \Vert ^{-1} ,\Vert U_0^{-1} \Vert ^{-1} ),$ and \[ \mu = \max \left( \Vert L_0^* \Vert \Vert L_0^{*-1} \Vert  \Vert U_0^{*-1} \Vert, \Vert U_0^* \Vert \Vert U_0^{*-1} \Vert  \Vert L_0^{*-1} \Vert \right).\]
\end{prop}

\textbf{Conclusion}

Consequently, we can apply Proposition \ref{prop:boundLambdaf2} and Proposition 3.12 of \cite{caruso-roe-vaccon:14a}: 

\begin{prop}
For any $\rho \in (0,1],$ if $r<e^{ \nu -2(\alpha +\mu)+\log \left( \frac{\rho}{C} \right)},$ the equation \ref{eq:firstorder} for $f$ is satisfied.
\end{prop}

\begin{ex}
If $\Vert L_0\Vert = \Vert L_0^{-1}\Vert =\Vert U_0\Vert =  \Vert U_0^{-1}\Vert = \rho=1$ and $p=2,$ then Proposition \ref{prop:precision} can be applied with $r=2^{-4}.$
\end{ex}

\subsubsection*{Numerical experiments}

\todo{Add numerical experiments...}
